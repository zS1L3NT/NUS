...
import {
  appendFilenameSuffix, canvasAssignmentOrder, canvasModuleOrder, collisionSafeNames,
  compareStates, documentRecord, htmlToMarkdown, orderPrefix, safeName,
  preserveIncompleteState, stateFromDocuments,
} from '../src/lib.mjs';
...
test('Canvas IDs are only added to colliding display names', () => {
  const items = [{ id: 1, name: 'Notes.pdf' }, { id: 2, name: 'notes.pdf' }, { id: 3, name: 'Slides.pdf' }];
  const names = collisionSafeNames(items, (item) => item.name, (item) => item.id);
  assert.equal(names.get(items[0]), 'Notes [Canvas 1].pdf');
  assert.equal(names.get(items[1]), 'notes [Canvas 2].pdf');
  assert.equal(names.get(items[2]), 'Slides.pdf');
});
...
test('module and assignment ordering follows Canvas position fields', () => {
  ...
  ], [{ id: 20, position: 2 }, { id: 10, position: 1 }]);
