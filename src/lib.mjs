...
export function orderPrefix(position, width = 3) {
...
export function collisionSafeNames(items, nameForItem, idForItem) {
  ...
  return new Map(items.map((item, index) => {
    ...
    const name = counts.get(key) > 1
      ? appendFilenameSuffix(baseName, `[Canvas ${idForItem(item)}]`)
      : baseName;
    return [item, name];
  }));
}
...
function positioned(items = []) {
  return items.map((item, index) => ({ item, index }))
    .sort((left, right) => {
      ...
      const positionDifference = (Number.isFinite(leftPosition) ? leftPosition : Number.MAX_SAFE_INTEGER)
...
export function canvasModuleOrder(modules = []) {
  ...
  positioned(modules).forEach((module, moduleIndex) => {
    ...
    for (const item of positioned(module.items || [])) {
      itemRank += 1;
      if (item.type === 'Page' && item.page_url && !pageRanks.has(String(item.page_url))) pageRanks.set(String(item.page_url), itemRank);
      if (item.type === 'Assignment' && item.content_id && !assignmentRanks.has(Number(item.content_id))) assignmentRanks.set(Number(item.content_id), itemRank);
      if (item.type === 'File' && item.content_id && !fileRanks.has(Number(item.content_id))) fileRanks.set(Number(item.content_id), itemRank);
    }
  });
  return { moduleRanks, pageRanks, assignmentRanks, fileRanks };
}

export function canvasAssignmentOrder(assignments = [], assignmentGroups = []) {
  const groupPositions = new Map(positioned(assignmentGroups).map((group, index) => [Number(group.id), index + 1]));
  const ordered = assignments.map((assignment, index) => ({ assignment, index }))
    .filter(({ assignment }) => groupPositions.has(Number(assignment.assignment_group_id)) && Number.isFinite(Number(assignment.position)))
    .sort((left, right) => groupPositions.get(Number(left.assignment.assignment_group_id)) - groupPositions.get(Number(right.assignment.assignment_group_id))
...
export function preserveIncompleteState(previous = {}, current = {}, incompleteKinds = []) {
